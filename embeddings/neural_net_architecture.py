...
class NNArtifact():
    ...
    def train_nn(self, training_set_size, train_x, train_y, val_x, val_y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(total_batch):
                batch_x, batch_y = self.batch_creator(self.batch_size, train_x.shape[0], train_x, train_y)
                _, c = self.sess.run([optimizer, cost], feed_dict={self.x: batch_x, self.y: batch_y})

            avg_cost += c / total_batch

            print("Epoch:", (epoch+1), "cost =", "{:.20f}".format(avg_cost))

        pred_temp = tf.equal(tf.argmax(self.output_layer, 1), tf.argmax(self.y, 1))
        accuracy = tf.reduce_mean(tf.cast(pred_temp, "float"))
        print("Validation Accuracy:", accuracy.eval(session=self.sess, feed_dict={self.x: val_x.reshape(-1, self.input_num_units), self.y: dense_to_one_hot(val_y)}))
    ...
    def predict_using_nn(self, test_x):
        ...
        pred = predict.eval(session=self.sess, feed_dict={self.x: test_x.reshape(-1, self.input_num_units)})
